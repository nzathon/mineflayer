...
function extractSkinInformation (properties) {
  ...
  if (!props.textures || !props.textures.value || !prob.value) {
    return undefined
  }

  const skinTexture = JSON.parse(Buffer.from(props.textures.value || props.value, 'base64').toString('utf8'))
  ...
  const skinTextureAvatar = 'https://mc-heads.net/avatar/' + skinTexture.split('/').pop() + '/nohelm'

  if (!skinTextureUrl) {
    return undefined
  }

  return { url: skinTextureUrl, model: skinTextureModel, avatar: skinTextureAvatar }
}
